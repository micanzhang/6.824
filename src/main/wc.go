...
func Map(value string) *list.List {
	l := list.New()
	words := strings.FieldsFunc(value, func(n rune) bool {
		return !unicode.IsLetter(n)
	})

	wmaps := make(map[string]int)
	for _, word := range words {
		wmaps[word] += 1
	}

	for word, count := range wmaps {
		l.PushBack(mapreduce.KeyValue{Key: word, Value: strconv.Itoa(count)})
	}
	return l
}
...
func Reduce(key string, values *list.List) string {
	...
	for elem != nil {
		...
		if mcount, err := strconv.Atoi(value); err == nil {
			count += mcount
		}
